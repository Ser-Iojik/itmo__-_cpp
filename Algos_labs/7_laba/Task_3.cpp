...
// ifstream fin("additionin.txt");
// ofstream fout("additionout.txt");
...
void insert(Node **head, int key, int left, int right) {
    ...
    while (tmp) {
        ...
        // Также обрабатываем левую ветвь
            ...
            exit(2); // Функция exit выполняет немедленное завершение программы. Завершаемый процесс, как правило, выполняет очистку используемой памяти. 
        }
    }
}
...
void print_tree(int N, Node* root) {
    fout << N+1 << endl; // печатаем кол-во вершин дерева
