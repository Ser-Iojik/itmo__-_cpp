...
class Stack {
    private: // методы и свойства будут только внутри класса
        StackNode *top; // Конкретно в этом классе берём указатель на верхний элемент
    public: // к ф-циям и методам доступ можно получить из других ф-ций и объектов программы
        Stack() {
        ...
        void push(int Num) {
            ...
            node->next = top; // a->b == (*a).b И указатель равен указателю на следующий элемент            
